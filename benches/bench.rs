...
use diff_pretty::pager::bench_internals::SearchViewport;
use diff_pretty::{render, render_document, render_reader_document, render_reader_to, render_to};
...
fn pgo_training_workload() -> usize {
    // Train on the same real-shape bundles that are reported individually,
    // with the ordinary show corpus carrying most of the work. Keep this
    // Composite workload keeps PGO focused on practical parser and renderer
    // branch frequencies.
    let mut bytes = render_typical_show_fixtures()
        .wrapping_add(render_named_fixtures(SHOW_METADATA_FIXTURES))
        .wrapping_add(render_named_fixtures(SHOW_LARGE_FIXTURES));
    bytes = bytes.wrapping_add(render(fixture("log_000")).len());
    bytes = bytes.wrapping_add(render(fixture("log_000_color")).len());
    bytes = bytes.wrapping_add(render(fixture("plain_unified")).len());

    let input = fixture("show_010");
    let document = render_document(input);
    let mut viewport = Vec::with_capacity(16 * 1024);
    document
        .write_viewport(&mut viewport, 0, 24)
        .expect("writing to a Vec cannot fail");
    let mut sink = std::io::sink();
    render_to(fixture("show_020"), &mut sink).expect("sink writes cannot fail");
    render_reader_to(fixture("log_000").as_bytes(), &mut sink).expect("sink writes cannot fail");
    let reader_document =
        render_reader_document(fixture("log_000").as_bytes()).expect("fixture reader cannot fail");
    let search = SearchViewport::new(&reader_document, "fn", 24, 80);
    bytes
        .wrapping_add(viewport.len())
        .wrapping_add(search.top())
}
...
#[divan::bench]
fn render_pgo_training_workload(b: Bencher) {
    b.bench_local(|| black_box(pgo_training_workload()));
}
